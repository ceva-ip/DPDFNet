...
class Encoder(Module):
    ...
    def forward(
        ...
        feat_erb_in = feat_erb[..., :-1] if self.drop_last_erb_bin else feat_erb
...
class ErbDecoder(Module):
    ...
    def forward(self, emb: Tensor, e3: Tensor, e2: Tensor, e1: Tensor, e0: Tensor) -> Tensor:
        ...
        if self.pad_last_erb_bin:
            # Mirror the highest bin to recover the full ERB-band count on the legacy path.
...
if __name__ == '__main__':
    waveform_8k = torch.randn(8000, dtype=torch.float32)[None, :]
    model = DPDFNet8KHz(dprnn_num_blocks=2)
    model.eval()
    print(model)
    model(waveform_8k)
    print('DPDFNet8KHz forward passed!')
