...
class MagnitudeMask(nn.Module):
    """Streaming mask for paths where mask is already per-frequency-bin."""
...
class Encoder(Module):
    def __init__(
        ...
        self.drop_last_erb_bin = drop_last_erb_bin
        ...
        erb_fc_emb = group_linear_layer(
            input_size=conv_ch * erb_conv_bins,
    ...
    def forward(
        ...
        feat_erb_in = feat_erb_buffered[..., :-1] if self.drop_last_erb_bin else feat_erb_buffered
        e0 = self.erb_conv0(feat_erb_in)
...
class ErbDecoder(Module):
    def __init__(
        ...
        self.pad_last_erb_bin = pad_last_erb_bin
        erb_conv_bins = erb_input_bins - 1 if drop_last_erb_bin else erb_input_bins
        erb_conv_bins = (erb_conv_bins + 2 - 3) // 2 + 1
        erb_conv_bins = (erb_conv_bins + 2 - 3) // 2 + 1
        ...
        erb_fc_emb = group_linear_layer(
            input_size=emb_dim,
            hidden_size=conv_ch * erb_conv_bins,
            groups=enc_lin_groups
        )
        self.erb_fc_emb = nn.Sequential(erb_fc_emb, nn.ReLU(inplace=True))
...
class DPDFNet8KHz(Module):
    def __init__(
        ...
        self.enc = Encoder(
            n_fft=n_fft,
            erb_input_bins=self.freq_bins,
            drop_last_erb_bin=True,
            emb_dim=emb_dim,
            nb_erb=nb_erb,
            nb_df=self.nb_df,
            conv_ch=conv_ch,
    ...
    def initial_state(
        ...
        if state is not None:
            state = state.reshape(-1)
            if state.numel() != self.state_size():
                raise ValueError(f"Initial state size mismatch in DPDFNet8KHz: expected {self.state_size()}, got {state.numel()}")
            return state
        chunks = [
...
def correct_state_dict(sd: dict) -> dict:
    streaming_sd = dict()
    for k, v in sd.items():
        # Streaming uses per-bin magnitude masks; legacy checkpoints may still carry ERB inverse FB.
...
if __name__ == '__main__':
    model = DPDFNet8KHz(dprnn_num_blocks=2)
    print(model)
    model.eval()
    state = model.initial_state(dtype=torch.float32)
    spec = torch.randn(1, 1, model.freq_bins, 2, dtype=torch.float32)
    with torch.no_grad():
        out, state = model(spec, state)
    print(tuple(out.shape), tuple(state.shape))
    print("DPDFNet8KHz streaming forward passed!")
